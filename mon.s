...
*	move.l #0xff3ffb,IMR | 全割り込みマスク /*Step7以降可能となったためコメントアウト*/
	move.l #0xff2ff9, IMR /* 変更Timer1割込み、UART2割込み許可 */
****************
** 送受信 (UART1) 関係の初期化
****************
...
****************
	...
	move.w #0xe10c, USTCNT2 | 送受信可能, パリティなし, 1 stop, 8 bit,| 送受信割り込み可
	move.l #uart2_interrupt, 0x114  /*送受信割り込みをレベル5に設定*/ 
...
***************
	...
	.equ SYSCALL_NUM_SKIPMT, 5 /* mon.s 1.4.5での追加 */
****************************************************************
*** プログラム領域
****************************************************************
.section .text
...
G0:	
	...
	cmp #0, %d1
	beq G0_2
	jsr GETSTRING22 /* d0が1ならばGETSTRINGを呼び出す(藤) */
	bra trap_end
G0_2:
	jsr GETSTRING
...
G1:	
	...
	cmp #0, %d1
	beq G1_2
	jsr PUTSTRING2 /* d0が1ならばGETSTRINGを呼び出す(藤) */
	bra trap_end
G1_2:
...
G4:
	...
	movem.l %D0-%D7/%A0-%A6,-(%SP) /* レジスタ復帰（藤） */
    cmp #5, %d0 /* mon.s での追加 */
	bne trap_end2
	jsr CALL_RP
trap_end2:
...
**********************************************
**********************************************
uart2_interrupt: /*冨田が制作　割り込み命令が来たら先に受信かチェック */
	movem.l %d0-%d7/%a0-%a6, -(%sp) /* レジスタ退避（冨田）*/
	move.w URX2, %d3 /*URX1をd3に格納（冨田）*/
	move.b %d3, %d2
	btst #13,%d3 /* 以下2行でURX1の13ビット目をチェック（中嶋）*/
	beq UART2_EXIT1/*13ビット目が0だったらジャンプ　０なら受信キューにデータがない*/
	move.l #1, %d1
	jsr INTERGET2 /*INTERGET2へ（中嶋）*/

UART2_EXIT1:
	move.w UTX2, %d0 /* UTX1をd0に格納（冨田）*/
	btst #15, %d0 /* 以下2行でUTX1の15ビット目をチェック（中嶋）*/
	beq UART2_EXIT2/*15ビットめが0ならジャンプ　0は送信キューが空じゃない*/
	move.l #1, %d1
	jsr INTERPUT2 /* INTERPUT2へ（中嶋）*/
...
***UART2用のINTERGET,PUTSTRING,とか
************************************************
PUTSTRING2:
	movem.l	%d1-%d5/%a0, -(%sp)	/* レジスタを退避 (佐倉) */

	cmpi.l	#0x00000001, %d1	/* chが1でなければ何もせず復帰 (佐倉) */
	bne	PUTSTRING_END2
	...
	cmpi.l	#0x00000000, %d3	/* 送信する0ならばマスク処理へ (佐倉) */
	beq	PUTSTRING_END_SKIP_UNMASK2
	
PUTSTRING_LOOP2:
	cmp.l	%d3, %d4		/* 送信したデータ数が指定したサイズと等しければマスク処理へ (佐倉) */
	beq	PUTSTRING_UNMASK2
	...
	addq	#1, %d4			/* 送信したデータ数を1加算 (佐倉) */
	addq	#1, %a0			/* 読み込み先アドレスを1加算 (佐倉) */
	bra	PUTSTRING_LOOP2		/* ループ先頭へ (佐倉) */
PUTSTRING_UNMASK2:
	move.w	USTCNT2, %d5		/* USTCNT1を操作し送信割り込み許可 (佐倉) */
	ori.w	#0x0004, %d5		/* USTCNT1の第2bit(TXEE)をセットし割り込みを許可 (島添) */
	move.w	%d5, USTCNT2
PUTSTRING_END_SKIP_UNMASK2:
	move.l	%d4, %d0		/* 送信したデータ数を%d0に転送 (佐倉) */
PUTSTRING_END2:	
	movem.l	(%sp)+, %d1-%d5/%a0	/* レジスタを復帰 (佐倉) */
	rts
...
INTERPUT2:
	...
	cmpi.l	#0x00000001, %d1	/* chが1でないなら何もせず復帰 (佐倉) */
	bne	INTERPUT_END2

	move.l	#0x00000001, %d0	/* 送信キューからデータを取り出す (佐倉) */
	jsr	OUTQ2			/* 取り出されたデータ=%d1.l (島添) */
					/* 成功：%d0.l=1, 失敗：%d0.l=0 (島添) */
	
	cmpi.l	#0x00000000, %d0	/* 取り出し失敗ならマスク処理して復帰 (佐倉) */
	bne	PUT2
	move.w	USTCNT2, %d2		/* USTCNT1を操作し送信割り込みをマスク (佐倉) */
	andi.w	#0xFFFB, %d2		/* USTCNT1の第2bit(TXEE)をクリアし割り込みをマスク (島添) */
	move.w	%d2, USTCNT2
	bra	INTERPUT_END2
PUT2:
...
** INTERGET
************************************************
INTERGET2: /* 入力d1:チャネルch　d2:受信データdata（島添）**/
	movem.l	%d0-%d2,-(%sp)	/* レジスタ退避（島添）**/
	
	cmpi	#1,%d1	/* チャネルが0であるか判定（島添）**/
	bne	INTERGET_Finish2	/* チャネル0以外の場合終了処理（島添）**/
	move.l	#0,%d0	/* INQで使用するキューの選択（佐倉）**/
	move.b	%d2,%d1	/* INQへの入力（佐倉）**/
	jsr	INQ2	/* INQ呼び出し（島添）**/
...
** GETSTRING
************************************************
GETSTRING22: /* 入力d1:チャネルch　d2：データ書き込みの先の先頭アドレスp　d3：取り出すデータサイズsize（島添）*/
	/* 出力d0：実際に取り出したデータサイズsz（島添）*/
	movem.l	%d1-%d4/%a0,-(%sp)	/* レジスタ退避（島添）**/
	cmpi.l	#1,%d1	/* チャネルが0であるか判定（島添）**/
	bne	GETSTRING_Finish2	/* チャネル0以外でGETSTRING終了処理へ（佐倉）**/
	move.l	%d2,%a0	/* i:データ書き込み先の先頭アドレス（佐倉）**/
	move	#0,%d4	/* sz：実際に取り出したデータサイズ（佐倉）**/
	
GETSTRING12:	
	cmp	%d3,%d4	/* 実際に取り出したデータサイズが入力d3:取り出すデータサイズに一致するか判定（島添）**/
	beq	GETSTRING2_2	/* 一致した場合d0にデータサイズを書き込んで終了処理（島添）**/

	move.l	#0x00000000, %d0	/* d0の初期化（島添）**/
	jsr	OUTQ2	/* OUTQを呼び出し、バッファに入っているデータの取り出し（島添）**/
	/* OUTQはd0に対して失敗時0、成功時1を出力する（佐倉）**/
	cmpi	#0,%d0	/* OUTQが失敗なら出力値を書き込み終了処理へ（島添）**/
	beq	GETSTRING2_2
	move.b	%d1,(%a0)+	/* p番地にコピー i++（島添）**/
	add	#1,%d4	/* sz++（島添）**/
	bra	GETSTRING12	/* GETSTRING1をループ（島添）**/
...
Q_Init2: /* 初期化（島添）*/
	...
	lea.l	BF_START2, %a0	/* キュー作成のために確保した領域の先頭アドレスを取得（島添）*/
	move.l	%a0, top2	/* キュー0の先頭アドレスの書き込み（島添）*/
	move.l	%a0, out2	/* キュー0のOUTQにて次にアクセスするアドレスを書き込み（島添）*/
	move.l	%a0, in2	/* キュー0に対するINQにて次にアクセスするアドレスを書き込み（島添）*/
	add	#255, %a0	/* キュー0の最後尾のアドレスを計算（島添）*/
	move.l	%a0, bottom2	/* キュー0の最後尾のアドレスを書き込み（島添）*/
	move.l	#0x00, s2	/* キュー0の保存されているデータ数を初期化（島添）*/
	
	lea.l	top2, %a1	/* 以下はキュー0と同じようにキュー1の初期化を行うための処理（島添）*/
	lea.l	out2, %a2
	lea.l	in2, %a3
	lea.l	bottom2, %a4
	lea.l	s2, %a5
...
INQ2:
	...
	lea.l	s2, %a0			/* サイズを比較し書き込み可能か判定 (佐倉) */
	move.l	(%a0, %d3), %d2		/* %d2に指定キューのサイズ(s+オフセット)を転送 (島添) */
	cmp	#256, %d2		/* サイズが256でなければ書き込み可能なので書き込み処理へ (島添) */
	bne	IN_STEP_12
	move.l	#0, %d0			/* 256ならば%d0.l=0として終了 (島添) */
...
IN_STEP_12:
	lea.l	in2, %a1			/* データ書き込み先アドレスを%a1に (佐倉) */
	lea.l	bottom2, %a4		/* キューの末尾アドレスを%a4に (佐倉) */
	lea.l	top2, %a5		/* キューの先頭アドレスを%a5に (佐倉) */
	movea.l	(%a1, %d3), %a3     	/* 指定キューの書き込み先アドレス(in+オフセット)のアドレスを%a3に (佐倉) */
	move.b	%d1, (%a3)         	/* データの書き込み (佐倉) */
...
OUTQ2:
	...
	lea.l	s2, %a0	/* s:保存されているデータ数を保持 先頭から4ビットはキュー0、その後ろ4ビットはキュー1（佐倉）*/
	move.l	(%a0, %d3), %d2	/* 今回扱うキューに保存されているデータ数をd2に書き込み（島添）*/
				/* (s+オフセット)で指定キューのs (佐倉) */
	cmp	#0, %d2		/* 保存されているデータが0であれば終了処理（島添）*/
	bne	OUT_STEP_12
...
OUT_STEP_12:
    	lea.l	out2, %a1	/* データ読み取り先のアドレスを保存しているメモリアドレスの取得（島添）*/
	lea.l	bottom2, %a4	/* それぞれのキューの末尾のアドレスを保存しているメモリアドレスの取得（島添）*/
	lea.l	top2, %a5	/* それぞれのキューの先頭のアドレスを保存しているメモリアドレスの取得（島添）*/
	...
	movea.l	(%a4, %d3), %a2	/* 今回扱っているキューの最後尾のアドレスを取得（島添）*/
				/* (bottom+オフセット)で指定キューのbottom (佐倉) */
	cmpa.l	%a3, %a2	/* 先程読み取った値が保存されていたのがキューの最後尾か判定（島添）*/
	bne	OUT_INC2	/* 最後尾でなければoutの値を1加算（島添）*/
...
.section .bss
.even
BF_START2:	.ds.b	512 | バッファ領域
top2:		.ds.l	2   | 先頭アドレス用領域
out2:		.ds.l	2   | 読み出し先アドレス格納用領域
in2:		.ds.l	2   | 書き込み先アドレス格納用領域
bottom2:		.ds.l	2   | 末尾アドレス格納用領域
s2:		.ds.l	2   | 格納されているデータ数格納用領域

BUF2:
.ds.b 256 | BUF2[256] | 作業用バッファ
.even
