...
if (timeout==0){
        fprintf(com0out,"時間ぎれ\n");
        fprintf(com0out,"正解は%s\n",answer_char);
        fprintf(com1out,"時間ぎれ\n");
        fprintf(com1out,"正解は%s\n",answer_char);
        } else if (answer_right1==1 && answer_right2==1){
     ...
     if (q_count == 6){
      ...
      if(p1_score == p2_score){
       ...
       fprintf(com1out,"引き分け\n");
        ...
        /* 回答権のロックを解除（スタート！） */
        V(1);
         for(volatile int i=0; i<time_count;i++) {/*時間制限*/
